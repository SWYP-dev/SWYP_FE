...
// SSR에서는 window가 없으므로 typeof 가드 필수 (없으면 서버 렌더링 단계에서 크래시).
...
/** parseWixMediaUrl가 분해해 낸 Wix Media URL 조각. */
interface ParsedWixMediaUrl {
  baseUrl: string;
  filename: string;
}

/** buildTransformUrl에 넘기는 변환 옵션. */
 ...
 */
function parseWixMediaUrl(src: string): ParsedWixMediaUrl | null {
  try {
...
const clampDim = (n: number): number => Math.min(Math.max(Math.round(n), 1), MAX_DIMENSION);
...
function buildSrcSet(parsed: ParsedWixMediaUrl, options: TransformOptions): string {
...
interface ResponsiveImageProps extends Omit<React.ComponentPropsWithoutRef<'img'>, 'src' | 'srcSet'> {
...
const ResponsiveImage = React.forwardRef<HTMLImageElement, ResponsiveImageProps>(
  (
    ...
    // <img>가 조건부 렌더링이라 첫 렌더에는 null이다. 의존성 배열 없이 매 렌더
    // 동기화해서 실제로 마운트된 시점의 엘리먼트가 부모 ref에 연결되도록 함.
    ...
    // `size` is undefined exactly once: the pre-measurement first render, which
    // we never let reach the network (see below — useSize measures before paint).
    ...
    const options: TransformOptions | undefined = size && {
...
const Image = React.forwardRef<HTMLImageElement, ImageProps>(
  (
    ...
    const parsed = imgSrc === FALLBACK_IMAGE_URL || !imgSrc ? null : parseWixMediaUrl(imgSrc);
